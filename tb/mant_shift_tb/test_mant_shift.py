...
def golden_reference(mantissa: int, shift: int):
    shift = min(shift, MAX_SHIFT)

    wide = SIG_WIDTH + MAX_SHIFT

    padded = mantissa << MAX_SHIFT  # this is done so data is not lost
    result_shift = padded >> shift

    mask_wide = (1 << wide) - 1
    result_shift &= mask_wide

    shifted_mant = (result_shift >> MAX_SHIFT) & ((1 << SIG_WIDTH) - 1)
    ...
    return shifted_mant, guard_bit, round_bit, sticky_bit


async def drive_and_check(dut, mant, shift, label=""):
    ...
    # expected values
    exp_shifted, exp_g, exp_r, exp_s = golden_reference(mant, shift)

    # dut values
    got_shifted = int(dut.mant_o.value)
    got_g = int(dut.guard_o.value)
    got_r = int(dut.round_o.value)
    got_s = int(dut.sticky_o.value)

    # assert results
    val = f"mant=0x{mant:06x} shift=0x{shift:06x}"
    assert (
        got_shifted == exp_shifted
    ), f"mant_o: got 0x{got_shifted:06x} expected 0x{exp_shifted:06x}  [{val}]"
...
async def test_direct_cases(dut):
    for name, mant, shift in DIRECTED_CASES:
        await drive_and_check(dut, mant, shift, name)
...
async def test_random(dut):
    ...
    for i in range(NUM_TESTS):
        mant = rand.randint(0, MAX_VAL)
        shift = rand.randint(0, MAX_SHIFT)
        await drive_and_check(dut, mant, shift)
